import { log, yellow, red, magenta } from './colors';
...
export const primaryErrors = () => {
    ...
    if (program.args.length === 0) {
        log(
            red.bold('Error : ') +
                red('You need to specify a ') +
                yellow.bold('name ') +
                red('for your component(s)'),
        );
        log('');
...
export const extraArgsErrors = () => {
    ...
    if (hasSpaces > 0) {
        log(
            red.bold('Error : ') +
                red('There should be ') +
                red.bold('no spaces after comma(s) ') +
                red('between components Names'),
        );
        log('');
        log(magenta('Exit process'));
    } else {
